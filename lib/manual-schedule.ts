...
/**
 * Validates privacy-minimal busy intervals read by a native device calendar.
 * Native code clips recurring and multi-day events to the invitation window;
 * the server checks that boundary again before trusting the payload.
 */
export function deviceCalendarIntervals(input: {
  intervals: unknown;
  eventStartDate: string;
  eventEndDate: string;
  timezone: string;
  ...
  if (input.intervals.length > 2_000) {
    throw new ManualScheduleValidationError("Too many busy times were submitted.");
  }
  ...
  const intervals = input.intervals.map((value, index): Interval => {
    if (
      !Array.isArray(value) ||
      value.length !== 2 ||
